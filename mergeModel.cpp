...
mergeModel::mergeModel(const QString &name, QObject *parent):QAbstractTableModel(parent)
{
    // m_itemName = name;
    m_db = QSqlDatabase::addDatabase("QSQLITE");
    m_db.setDatabaseName(name+".db");
    if(!m_db.open())
    {
        qDebug() << "Falied to open database!"<<m_db.lastError().text() ;
    }
...
void mergeModel::initTable(const QString &tableName)
{
    ...
    for (int row = 0; row < gridSize; ++row) {
        for (int col = 0; col < gridSize; ++col) {
            query.bindValue(0, "");  // Empty value for now
...
void mergeModel::removeTable(const QString &tableName)
{
    QSqlQuery query;
    ...
    if (!query.exec(dropTableQuery)) {
        qDebug() << "Failed to drop table:" << query.lastError();
    } else {
...
TableState *mergeModel::getState()
{
    return &m_state;
}
